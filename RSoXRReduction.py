...
class RSoXRProcessor:
    ...
    def reduce_group_with_scaling(self, group_number=None, scans=None, trims=None, 
                                ...
                                save_data=True, output_filename=None, log_scale=True, 
                                use_open_beam=None):
        """
        ...
        output_dir : str, optional
            Directory to save plots and data files
        plot_prefix : str, optional
            Prefix for plot filenames  
        save_data : bool, optional
            Whether to save the processed data to file (default: True)
        output_filename : str, optional
            Custom filename for saved data. If None, auto-generates from group/energy info
        ...
        result = processor.reduce_group_with_scaling(
            ...
            energy=285.0,
            plot=True,
            output_filename='my_sample_285eV.dat'  # Custom filename
        )
        ...
        if save_data:
            self._save_processed_data(refl_smooth if smooth_data else raw_refl, 
                                    energy, output_dir, output_filename, 
                                    scaling_factors, backgrounds, trims)
    ...
    def reduce_group_with_scaling(self, group_number=None, scans=None, trims=None, 
        ...
        # Convert angle to q-space
        # Q = 4π sin(θ) / λ, where λ is wavelength in Angstroms
        wavelength_nm = 1239.9 / energy  # Convert eV to nm
        wavelength_angstrom = wavelength_nm * 10  # Convert nm to Angstroms
        q_values = 4 * np.pi * np.sin(np.radians(refl[:, 0])) / wavelength_angstrom  # Q in Å⁻¹
        
        # Create q-space data array
        q_data = np.column_stack([q_values, refl[:, 1]])
        print(f"Converted to q-space: q range {q_data[:, 0].min():.4f} to {q_data[:, 0].max():.4f} Å⁻¹")
        
        # Store raw data before smoothing
        raw_q_data = deepcopy(q_data)
        
        # Apply smoothing if requested
        if smooth_data:
            if len(q_data) >= savgol_window:
                print(f"Applying Savitzky-Golay smoothing: window={savgol_window}, order={savgol_order}")
                smoothed_intensity = savgol_filter(q_data[:, 1], savgol_window, savgol_order)
                q_data_smooth = deepcopy(q_data)
                q_data_smooth[:, 1] = smoothed_intensity
            else:
                print(f"Warning: Data length ({len(q_data)}) < window size ({savgol_window}). Skipping smoothing.")
                q_data_smooth = deepcopy(raw_q_data)
        
        # Generate plots if requested (still show individual scans in angle space for visualization)
        if plot:
            self._plot_group_processing_results(all_scans, q_data_smooth if smooth_data else raw_q_data, 
                                            backgrounds, energy, scaling_factors, 
                                            output_dir=output_dir, plot_prefix=plot_prefix, 
                                            log_scale=log_scale, use_open_beam=use_open_beam,
                                            open_beam_intensity=open_beam_intensity)
        ...
        # Save data to file if requested
        if save_data:
            self._save_processed_data(q_data_smooth if smooth_data else raw_q_data, 
                                    energy, output_dir, output_filename, 
                                    scaling_factors, backgrounds, trims)
        
        # Return processed data in q-space
        if smooth_data:
            return q_data_smooth, raw_q_data
        else:
            return raw_q_data
    ...
    def _plot_group_processing_results(self, all_scans, final_data, backgrounds, energy, scaling_factors,
        ...
        try:
            ...
            print(f"• Manual scaling factors: {scaling_factors}")
            print(f"• Final data points: {len(final_data)}")
            print(f"• q range: {final_data[:, 0].min():.4f} to {final_data[:, 0].max():.4f} Å⁻¹")
            if use_open_beam:
                print(f"• Open beam normalization applied (I₀ = {open_beam_intensity:.3e})")
    ...
    def _save_processed_data(self, data, energy, output_dir=None, output_filename=None, 
                            scaling_factors=None, backgrounds=None, trims=None):
        """
        Save processed q-space data to file in simple 3-column format.
        
        Parameters:
        -----------
        data : numpy array
            Processed data to save (q, intensity)
        energy : float
            Photon energy in eV
        output_dir : str, optional
        ...
        scaling_factors : list, optional
            Scaling factors used in processing (not saved, for info only)
        backgrounds : list, optional
            Background values used in processing (not saved, for info only)
        trims : list, optional
            Trim values used in processing (not saved, for info only)
        """
        try:
            ...
            # Calculate error estimates (1% of intensity)
            errors = 0.01 * np.abs(data[:, 1])
            
            # Create 3-column data array
            output_data = np.column_stack([data[:, 0], data[:, 1], errors])
            
            # Save data with simple header
            with open(output_path, 'w') as f:
                # Write column headers only
                f.write("q\tintensity\terror\n")
                
                # Write data
                for q, intensity, error in output_data:
                    f.write(f"{q:.6f}\t{intensity:.6e}\t{error:.6e}\n")
            
            print(f"Data saved to: {output_path}")
            print(f"Format: 3 columns (q, intensity, error)")
            return output_path
            
        except Exception as e:
            print(f"Warning: Could not save data file: {str(e)}")
            return None
